...
export default function VideoPlayer({ url, title, playerConfig = {}, episodeId, startAt = 0, forceShowControls = false }) {
  const {
  ...
  const resetHideTimer = useCallback(() => {
    ...
    if (playing && hideDelay > 0 && !forceShowControls) {
      hideTimer.current = setTimeout(() => setShowControls(false), hideDelay)
    }
  }, [playing, hideDelay, forceShowControls])

  useEffect(() => { resetHideTimer(); return () => clearTimeout(hideTimer.current) }, [playing, resetHideTimer])

  // En cuanto termina el giro de pantalla, vuelve a evaluar si debe ocultar los controles
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onMeta  = () => {
      ...
      // Retomar desde donde se quedó, si se indicó un punto de inicio
  ...
  // y también al pausar o al desmontar el componente (salir de la página)
  useEffect(() => {
    ...
    const persist = () => {
      if (v.duration) saveProgress(episodeId, v.currentTime, v.duration)
    }
    ...
    return () => {
      ...
      persist() // guarda una última vez al salir
    }
  }, [isNative, episodeId])
  ...
  return (
    ...
    >
      ...
      {!playing && <div className={styles.centerPlay}><IconPlay /></div>}

      <div className={`${styles.controls} ${showControls ? styles.visible : ''}`}
        style={{ background: controlsBg }}
        onClick={e => e.stopPropagation()}
      >
        {/* ── Fila 1: barra de progreso, ocupa todo el ancho ── */}
        ...
        {/* ── Fila 2: botones de control, debajo de la barra ── */}
        <div className={styles.btnRow}>
          ...
          <button className={styles.ctrlBtn} onClick={() => !document.fullscreenElement ? containerRef.current?.requestFullscreen?.() : document.exitFullscreen?.()} tabIndex={-1}>
