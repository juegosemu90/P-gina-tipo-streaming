...
export default function VideoPlayer({ url, title, playerConfig = {}, onNearEnd }) {
  const {
  ...
  // Reiniciar el flag al cambiar de video
  useEffect(() => { nearEndFired.current = false }, [url])
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      ...
      // Disparar cuando falte 1 minuto (60s) para terminar
      if (!nearEndFired.current && v.duration && (v.duration - v.currentTime) <= 60 && (v.duration - v.currentTime) > 0) {
