...
export default function VideoPlayer({ url, title }) {
  ...
  // Guarda el touchIdentifier para ignorar otros toques
  ...
  const onProgressStart = (e) => {
    ...
    // Guardar qué dedo inició el arrastre
    ...
    const onMove = (ev) => {
      if (!isDragging.current) return
      // En touch: solo procesar el dedo que inició
      if (ev.touches) {
        const touch = Array.from(ev.touches).find(t => t.identifier === dragTouchId.current)
        if (!touch) return
      }
  ...
  const handleContainerClick = () => {
    // No hacer nada si justo terminó un arrastre
    if (isDragging.current) return
    if (!isNative) return
    ...
    >
      ...
      >
        <div className={styles.btnRow}>
          ...
          {/* Barra de progreso — solo responde al dedo que la inició */}
          <div
            className={styles.progressWrap}
            ref={progressRef}
            onMouseDown={onProgressStart}
            onTouchStart={onProgressStart}
          >
