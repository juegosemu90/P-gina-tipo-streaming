...
export default function Inicio({ playerConfig, searchOpen, setSearchOpen }) {
      ...
      <div className={styles.detailWrapper}>
        ...
        <div className={styles.detailInfo}>
          ...
          <p className={styles.detailMeta}>
            Página {activePage} · {pageEpisodes.length} episodio{pageEpisodes.length !== 1 ? 's' : ''}
          </p>
          {firstEp && (
        ...
        <div className={styles.seasonPicker}>
          {PAGES.map(p => (
            <button
              key={p}
              className={`${styles.seasonPickerBtn} ${activePage === p ? styles.seasonPickerActive : ''}`}
              style={activePage === p ? { background: PAGE_COLORS[((p - 1) % 7) + 1], borderColor: PAGE_COLORS[((p - 1) % 7) + 1] } : {}}
              onClick={() => setActivePage(p)}
            >
              Página {p}
              {activePage === p && <Check size={11} />}
            </button>
          ))}
        </div>

        <div className={styles.episodeListLabel} style={{ color }}>PÁGINA {activePage}</div>
            ...
            {pageEpisodes.map(ep => {
              ...
              return (
                <div key={ep.id} className={`${styles.episodeRow} ${sched ? styles.episodeRowScheduled : ''}`} onClick={() => openEpisode(ep)}>
                  <span className={styles.episodeEpNum} style={{ color }}>
                    EP {ep.episode || '?'}
                  </span>
                  ...
                  <div className={styles.episodeInfo}>
                    <div className={styles.episodeTitleRow}>
                      ...
                      {sched && isAdmin && <Lock size={11} className={styles.episodeLockIcon} />}
                    </div>
                    {ep.duration && <div className={styles.episodeDate}>{ep.duration}</div>}
                    {ep.description && <p className={styles.episodeDesc}>{ep.description}</p>}

                    <button className={styles.watchNowBtn} style={{ background: color }} onClick={e => { e.stopPropagation(); openEpisode(ep) }}>
  ...
  return (
    <div className={styles.wrapper}>
      ...
      {!searchOpen && (
        <>
          {/* ── BOTONES DE PÁGINA — rectangulares y de color ── */}
          <section className={styles.seasonSection}>
            <div className={styles.sectionHeader}>
              <span className={styles.sectionTag}>▣</span>
              <h2 className={styles.sectionTitle}>Páginas</h2>
            </div>
            <div className={styles.seasonGrid}>
              {PAGES.map(p => {
                const color = PAGE_COLORS[((p - 1) % 7) + 1]
                return (
                  <button
                    key={p}
                    ...
                    onClick={() => setActivePage(p)}
                  >
                    <span className={styles.seasonBtnNum}>Página {p}</span>
                  </button>
                )
              })}
            </div>
