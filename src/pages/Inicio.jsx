...
export default function Inicio() {
  ...
  // Detectar si la URL tiene un episodio al cargar
  useEffect(() => {
    if (episodes.length === 0) return
    const hash = window.location.hash
    const match = hash.match(/#\/episodio\/(.+)/)
    if (match) {
      const ep = episodes.find(e => e.id === match[1] || e.episode === match[1])
      if (ep) setSelected(ep)
    }
  }, [episodes])

  // Cambiar URL y título al seleccionar episodio
  const openEpisode = (ep) => {
    setSelected(ep)
    // URL con el número de episodio si tiene, si no el ID
  ...
  // Arrastre horizontal del slider
  useEffect(() => {
  ...
  if (selected) return <EpisodePlayer episode={selected} onBack={closeEpisode} />
