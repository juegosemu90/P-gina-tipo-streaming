  ...
  if (activePage !== null) {
    ...
    return (
      <div className={styles.detailWrapper}>
        ...
        {/* ── Poster grande con play centrado ── */}
        {firstEp && (
          <button className={styles.heroPoster} onClick={() => openEpisode(firstEp)}>
            {banner
              ? <img src={banner} alt="" className={styles.heroPosterImg} />
              : <div className={styles.heroPosterFallback} style={{ background: `linear-gradient(135deg, ${color}33, #000)` }} />
            }
            <div className={styles.heroPosterShade} />
            <div className={styles.heroPlayCircle} style={{ borderColor: color }}>
              <Play size={26} fill="#fff" style={{ marginLeft: 3 }} />
            </div>
            <div className={styles.heroRatingRow}>
              <span className={styles.heroRatingBadge}>5.0 ★</span>
              <span className={styles.heroAgeBadge}>B</span>
            </div>
          </button>
        )}
        ...
        <div className={styles.actionRow}>
          ...
          <button className={styles.actionBtn}>
            <span className={styles.actionIcon}>5 ★</span>
            CALIFICAR
          </button>
        </div>

        {/* ── Selector de bloque, solo si hay más de uno ── */}
        <button
          className={styles.blockHeaderRow}
          onClick={() => {
            if (PAGES.length > 1) {
              const next = activePage < PAGES.length ? activePage + 1 : 1
              setActivePage(next)
            }
          }}
        >
          <span className={styles.blockHeaderLabel} style={{ color }}>
            BLOQUE {activePage}{PAGES.length > 1 ? ` DE ${PAGES.length}` : ''}
          </span>
          {PAGES.length > 1 && <ChevronRight size={18} style={{ color }} />}
        </button>
