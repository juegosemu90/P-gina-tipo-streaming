...
export default function Inicio({ playerConfig }) {
  ...
  // Cargar episodios y refrescar cada 5 minutos
  useEffect(() => {
    ...
    fetchEpisodes()
    // Refrescar cada 5 minutos para liberar episodios automáticamente
    const interval = setInterval(fetchEpisodes, 5 * 60 * 1000)
    return () => clearInterval(interval)
  }, [])

  // Comprueba si un episodio todavía NO debe mostrarse
  const isScheduled = (ep) => {
    if (!ep.publishAt) return false
    const pub = ep.publishAt.toDate
      ? ep.publishAt.toDate()
      : new Date(ep.publishAt)
    return pub > new Date() // true = aún no es hora
  }
