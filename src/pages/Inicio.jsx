...
import { Play, Calendar, Lock, Search, X, ArrowLeft } from 'lucide-react'
...
// Encuentra el siguiente episodio según temporada y número de episodio
function findNextEpisode(list, current) {
  ...
  // Si no hay más en esta temporada, busca el primero de la siguiente temporada
...
export default function Inicio({ playerConfig, searchOpen, setSearchOpen, onEpisodeOpen }) {
  const [episodes,    setEpisodes]    = useState([])
  const [loading,     setLoading]     = useState(true)
  const [selected,    setSelected]    = useState(null)
  const [isAdmin,     setIsAdmin]     = useState(false)
  // searchOpen viene del padre
  ...
  return (
    <div className={styles.wrapper}>
      ...
      {/* ── LUPA — siempre visible en navbar ── */}
      {!searchOpen && (
      ...
      {/* ── PANTALLA DE BÚSQUEDA — reemplaza el contenido ── */}
      {searchOpen && (
        <div className={styles.searchScreen}>
          {/* Barra */}
          <div className={styles.searchBar}>
            <button className={styles.searchBack} onClick={closeSearch}>
              <ArrowLeft size={20} />
            </button>
                ...
                <div key={ep.id} className={styles.searchResult} onClick={() => openEpisode(ep)}>
                  ...
                  <div className={styles.searchInfo}>
                    {(ep.season || ep.episode) && (
                      <div className={styles.searchEpLabel}>T{ep.season} · Ep.{ep.episode}</div>
                    )}
