...
export default function EpisodePlayer({ episode, onBack, playerConfig = {} }) {
  ...
  // Revisa si hay progreso guardado de este episodio al entrar
  const [savedProgress, setSavedProgress] = useState(() => getProgress(episode.id))
  // null = aún no decide; mientras tanto se muestra el modal si hay progreso guardado
  const [startAt, setStartAt] = useState(null)

  useEffect(() => {
    if (!savedProgress) {
      // No hay nada guardado, empieza desde el inicio sin preguntar
      setStartAt(0)
    }
  }, [savedProgress])
