...
export default function Temporadas({ playerConfig }) {
  ...
  // Filtrar por temporada activa y ordenar por número de episodio
  const seasonEpisodes = visibleEpisodes
    .filter(ep => Number(ep.season) === activeSeason)
  ...
  // Contar episodios por temporada para el badge
  const countBySeason = (s) => visibleEpisodes.filter(ep => Number(ep.season) === s).length
