...
export default function Admin({ user, onLogout }) {
  ...
  const getPublishStatus = (ep) => {
    if (!ep.publishAt) return { label: 'Publicado', color: '#4ade80', scheduled: false }
    ...
    return { label: 'Publicado', color: '#4ade80', scheduled: false }
  }

  const formatPublishDate = (ep) => {
    if (!ep.publishAt) {
      const d = ep.createdAt?.toDate ? ep.createdAt.toDate() : new Date()
      return d.toLocaleDateString('es-MX', { day: 'numeric', month: 'short', year: 'numeric' })
    }
    const pub = ep.publishAt.toDate ? ep.publishAt.toDate() : new Date(ep.publishAt)
    return pub.toLocaleDateString('es-MX', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })
  }

  const published  = episodes.filter(ep => !getPublishStatus(ep).scheduled)
  const scheduled  = episodes.filter(ep => getPublishStatus(ep).scheduled)
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.scheduleBox}>
          <div className={styles.scheduleLabel}><Clock size={14} /> Programar publicación</div>
          <p className={styles.scheduleHint}>Déjalo vacío para publicar ahora mismo.</p>
        ...
        <textarea className={styles.textarea} name="description" placeholder="Descripción del episodio" value={form.description} onChange={handleChange} rows={3} />
      ...
      {/* Episodios programados — solo visible para admin */}
      {scheduled.length > 0 && (
        <div className={styles.card}>
          <h2 className={styles.cardTitle} style={{ color: '#f5c842' }}>
            <Clock size={16} /> Programados ({scheduled.length})
          </h2>
            ...
            {scheduled.map(ep => (
              <div key={ep.id} className={`${styles.epRow} ${editingId === ep.id ? styles.epRowActive : ''}`}>
                ...
                <div className={styles.epInfo}>
                  ...
                  <div className={styles.epStatus} style={{ color: '#f5c842' }}>
                    ● {getPublishStatus(ep).label}
                  </div>
                </div>
      ...
      <div className={styles.card}>
        ...
        {published.length === 0 ? (
          <p className={styles.empty}>No hay episodios publicados aún</p>
        ) : (
          <div className={styles.list}>
            {published.map(ep => (
              <div key={ep.id} className={`${styles.epRow} ${editingId === ep.id ? styles.epRowActive : ''}`}>
                ...
                <div className={styles.epInfo}>
                  ...
                  <div className={styles.epMeta}>
                    {ep.season && `T${ep.season}·E${ep.episode} · `}{ep.duration && `${ep.duration} · `}
                    Subido: {formatPublishDate(ep)}
                  </div>
                </div>
                <div className={styles.epActions}>
                  <button className={styles.editBtn} onClick={() => handleEdit(ep)}><Pencil size={14} /></button>
