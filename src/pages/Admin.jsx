...
import { Trash2, Plus, LogOut, Pencil, X, Check, Clock, Upload, Palette, Play, Image as ImageIcon } from 'lucide-react'
...
export default function Admin({ user, onLogout }) {
  ...
  const fetchConfig = async () => {
    try {
      ...
      const sb = await getDoc(doc(db, 'config', 'seasonBanners'))
      if (sb.exists()) setSeasonBanners(sb.data())
    } catch (e) {}
  }
  ...
  const saveSeasonBanners = async () => {
    try {
      await setDoc(doc(db, 'config', 'seasonBanners'), seasonBanners)
      showSaved('¡Imágenes de temporada guardadas!')
    } catch (e) { alert('Error: ' + e.message) }
  }
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.content}>
        ...
        {/* ── IMÁGENES DE BANNER POR TEMPORADA ── */}
        {tab === 'tema' && (
          <div className={styles.card}>
            <h2 className={styles.cardTitle}><ImageIcon size={15}/> Imagen del banner por temporada</h2>
            <p className={styles.bannerHint}>
              Pega el link de una imagen para cada temporada. Si la dejas vacía, se usará automáticamente la miniatura del primer episodio.
            </p>

            {[1,2,3,4,5,6,7].map(s => (
              <div key={s} className={styles.bannerRow}>
                <div className={styles.bannerPreview}>
                  {seasonBanners[s]
                    ? <img src={seasonBanners[s]} alt={`Temporada ${s}`} className={styles.bannerPreviewImg} />
                    : <div className={styles.bannerPreviewEmpty}>T{s}</div>
                  }
                </div>
                <input
                  className={styles.input}
                  placeholder={`URL de imagen para Temporada ${s}`}
                  value={seasonBanners[s] || ''}
                  onChange={e => setSeasonBanners(b => ({ ...b, [s]: e.target.value }))}
                />
                {seasonBanners[s] && (
                  <button className={styles.bannerClearBtn} onClick={() => setSeasonBanners(b => { const n = { ...b }; delete n[s]; return n })}>
                    <X size={13}/>
                  </button>
                )}
              </div>
            ...
            <button className={styles.submitBtn} onClick={saveSeasonBanners}>💾 Guardar imágenes de temporada</button>
          </div>
        )}
